...
# If quest.dia.line_time is 0 and dialogue is still ongoing
$execute as @s at @s if score @s quest.dia.line_time matches 0 run function lt:quest/data/$(quest_id)/dialogue/$(dialogue_id) with gamedata lt:quest/active_dialogue @s[type=player]
