...
execute as @e[tag=golfCrab] on owner run title @s actionbar [{"translate":"lt.golf.seconds_left","color":"dark_gray"},{"score":{"name":"@e[sort=nearest,tag=golfCrab,limit=1]","objective":"golf.timer"},"color":"yellow"},{"text":" | "},{"translate":"lt.golf.hits","color":"dark_gray"},{"score":{"name":"@e[sort=nearest,tag=golfCrab,limit=1]","objective":"golf.hits"},"color":"green"}]
execute as @e[tag=golfCrab] on owner if entity @s[tag=!in.golf.area] run function lt:world_games/minigolf/core/leave_hole with entity @s
