...
$execute if entity @s[tag=!golfInGame] if entity @a[tag=hole$(hole)Player,current_world=true] run tellraw @p[tag=!hole$(hole)Player] ["",{"selector":"@p[tag=hole$(hole)Player]"},{"translate":"lt.golf.in_use","color":"#ED8754"}]
