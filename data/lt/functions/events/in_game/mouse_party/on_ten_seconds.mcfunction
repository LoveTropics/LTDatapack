...
execute at @e[type=marker,tag=events.in_game.mouse_party.tracker] as @a[distance=..20,nbt={ForgeCaps:{"ltminigames:player_disguise":{entity:{type:"tropicraft:slender_harvest_mouse"}}}}] run scoreboard players add Players events.mouse_party 1
