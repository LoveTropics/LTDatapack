...
execute at @e[type=marker,tag=events.in_game.parrot_party.location] as @a[distance=..20,nbt={ForgeCaps:{"ltminigames:player_disguise":{entity:{type:"minecraft:parrot"}}}}] run scoreboard players add Players events.parrot_party 1
