# Get color 1
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color1 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 2
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color2 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 3
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color3 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 4
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color4 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 5
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color5 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 6
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color6 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 7
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color7 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 8
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color8 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 9
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color9 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 10
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color10 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 11
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color1 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color

# Get Color 12
tag @e[tag=p.cb.hit_detection,sort=random,limit=1] add p.cb.get_color
scoreboard players operation color12 p.cb.order = @e[tag=p.cb.get_color,limit=1] p.cb.color
tag @e[tag=p.cb.hit_detection] remove p.cb.get_color
